...
export const classesApi = {
  ...
  async getCategories(): Promise<ClassCategory[]> {
    const res = await api.get<any>('/api/v1/tenant/class-categories/');
    return res.data?.results || res.data || [];
  },

  async createCategory(data: Partial<ClassCategory>): Promise<ClassCategory> {
    const res = await api.post<ClassCategory>('/api/v1/tenant/class-categories/', data);
    return res.data;
  },

  // --- Class Templates ---
  async getTemplates(): Promise<ClassTemplate[]> {
    const res = await api.get<any>('/api/v1/tenant/class-templates/');
    return res.data?.results || res.data || [];
  },

  async createTemplate(data: Partial<ClassTemplate>): Promise<ClassTemplate> {
    const res = await api.post<ClassTemplate>('/api/v1/tenant/class-templates/', data);
    return res.data;
  },

  async updateTemplate(id: string, data: Partial<ClassTemplate>): Promise<ClassTemplate> {
    const res = await api.patch<ClassTemplate>(`/api/v1/tenant/class-templates/${id}/`, data);
    return res.data;
  },
  ...
  async getScheduleRules(branchId?: string): Promise<ClassScheduleRule[]> {
    ...
    const res = await api.get<any>('/api/v1/tenant/class-schedule-rules/', { params });
    return res.data?.results || res.data || [];
  },

  async createScheduleRule(data: Partial<ClassScheduleRule>): Promise<ClassScheduleRule> {
    const res = await api.post<ClassScheduleRule>('/api/v1/tenant/class-schedule-rules/', data);
    return res.data;
  },

  async generateOccurrencesFromRule(ruleId: string, fromDate: string, toDate: string): Promise<any> {
    const res = await api.post<any>(`/api/v1/tenant/class-schedule-rules/${ruleId}/generate-occurrences/`, {
      from_date: fromDate,
      to_date: toDate,
    });
    return res.data;
  ...
  async getOccurrences(filters?: { branch_id?: string; occurrence_date?: string; status?: string }): Promise<ClassOccurrence[]> {
    const res = await api.get<any>('/api/v1/tenant/class-occurrences/', { params: filters });
    return res.data?.results || res.data || [];
  },

  async createOccurrence(data: Partial<ClassOccurrence>): Promise<ClassOccurrence> {
    const res = await api.post<ClassOccurrence>('/api/v1/tenant/class-occurrences/', data);
    return res.data;
  },

  async assignTrainer(occurrenceId: string, trainerProfileId: string, trainerRole: string = 'LEAD'): Promise<any> {
    const res = await api.post<any>(`/api/v1/tenant/class-occurrences/${occurrenceId}/assign-trainer/`, {
  ...
  async assignContent(occurrenceId: string): Promise<any> {
    const res = await api.post<any>(`/api/v1/tenant/class-occurrences/${occurrenceId}/assign-content/`);
    return res.data;
  },

  // --- Content Studio ---
  async getContentItems(): Promise<ClassContentItem[]> {
    const res = await api.get<any>('/api/v1/tenant/class-content-items/');
    return res.data?.results || res.data || [];
  },

  async createContentItem(data: Partial<ClassContentItem>): Promise<ClassContentItem> {
    const res = await api.post<ClassContentItem>('/api/v1/tenant/class-content-items/', data);
    return res.data;
  },
  ...
  async getDemandRuns(branchId?: string): Promise<ClassDemandPlanningRun[]> {
    const params = branchId ? { branch_id: branchId } : {};
    const res = await api.get<any>('/api/v1/tenant/class-demand-planning-runs/', { params });
    return res.data?.results || res.data || [];
  },

  async getRecommendations(runId?: string): Promise<ClassScheduleRecommendation[]> {
    const params = runId ? { planning_run_id: runId } : {};
    const res = await api.get<any>('/api/v1/tenant/class-schedule-recommendations/', { params });
    return res.data?.results || res.data || [];
  },
};
